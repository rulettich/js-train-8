...
function combineAndReverseArrays(arr1, arr2) {
  ...
  if (!Array.isArray(arr1) || !Array.isArray(arr2)) {
    console.error("Один або обидва аргументи не є масивами");
    return [];
  }
...
function copyAndSwapElements(arr, target, start, end) {
  ...
  if (!Array.isArray(arr)) {
    console.error("Вхідний об'єкт не є масивом");
    return [];
  }
...
function customUnshift(arr, ...elements) {
  ...
  if (!Array.isArray(arr)) {
    return { initialLength: 0, newLength: 0, arr: [] };
  }
...
function customIncludes(arr, element) {
  ...
  const isPresent = arr.includes(element);
  const occurrenceCount = arr.filter((el) => el === element).length;
